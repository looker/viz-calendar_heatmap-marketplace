...
class CalendarHeatmap extends React.Component {
   ...
   drawGlobalOverview() {
    ...
    let item_block = this.items.selectAll('.item-block-year')
      ...
      .on('mousemove', d => {
        ...
        let tooltip_html = ''
        tooltip_html += '<div style="font-size:.8rem;"><span><strong>Total:  </strong></span>'
  ...
  drawYearOverview() {
    ...
    this.items.selectAll('.item-circle')
      ...
      .on('mousemove', d => {
        ...
        tooltip_html += `<div class="${styles.header}" style="font-size:.8rem;"><strong>${d.total ? d.total : 'No entries for '} </strong>`
        tooltip_html += '<span><strong> Total  ' + this.props.measure + '</strong></span></div><br>'
        tooltip_html += '<div style="font-size:.8rem;">' + moment(d.date).format('dddd, MMM Do YYYY') + '</div>'
        ...
        // console.log(Math.round(Number(d3.select("#tooltip").style('width').slice(0, -2))));
        this.tooltip
          .style("left", event.pageX-(Math.round(Number(this.tooltip.style('width').slice(0, -2)))/2)-5+"px")
    ...
    item_block.selectAll('.item-block-rect')
      ...
      .on('mousemove', d => {
       ...
       // tooltip_html += '<div><strong>' + (d.value ? this.formatTime(d.value) : 'No time') + ' tracked</strong></div>'
        tooltip_html += '<div style="font-size:.8rem;"><strong>' + (d.value ? d.value : 'No entries for')+'   '+this.props.measure+' </strong></div>'
        //tooltip_html += '<span><strong> Total  ' + this.props.measure + '</strong></span></div>'
        tooltip_html += '<div style="font-size:.8rem;">' + moment(date).format('dddd, MMM Do YYYY') + '</div>'
  ...
  drawWeekOverview() {
    ...
    let item_gutter = this.settings.item_gutter
    item_block.selectAll('.item-block-week')
      .data(d => d.summary)
      .enter()
      .append('rect')
      .attr('class', 'item item-block-week')
